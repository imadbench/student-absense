    ...
    switch ($action) {
        ...
        case 'save_attendance_participation':
            ...
            try {
                ...
                jsonResponse(false, [], 'Error saving attendance and participation data: ' . $e->getMessage());
            }
            break;
            
        case 'save_all_sessions_data':
            requireRole('professor');
            ...
            if (!$stmt->fetch()) {
                jsonResponse(false, [], 'Access denied. You do not teach this course.');
            }
            
            // Handle JSON data for all sessions
            $all_sessions_data = [];
            if (isset($_POST['sessions_data'])) {
                $all_sessions_data = json_decode($_POST['sessions_data'], true) ?: [];
            }
            
            if (empty($all_sessions_data)) {
                jsonResponse(false, [], 'No data to save');
            }
            ...
            try {
                ...
                if ($course && !empty($course['group_id'])) {
                    // Get enrolled students first
                    $stmt = $pdo->prepare("SELECT u.user_id, u.student_id FROM enrollments e
                                           JOIN users u ON e.student_id = u.user_id
                                           WHERE e.course_id = ?");
                    $stmt->execute([$course_id]);
                    $enrolled = $stmt->fetchAll();
                    ...
                    // If no enrolled students, get from same group
                    if (empty($valid_students)) {
                        $stmt = $pdo->prepare("SELECT DISTINCT u.user_id, u.student_id FROM users u
                ...
                // Fallback to all students if still empty
                if (empty($valid_students)) {
                    $stmt = $pdo->prepare("SELECT u.user_id, u.student_id FROM users u WHERE u.role = 'student'");
                    $stmt->execute();
                    $all_students = $stmt->fetchAll();
                ...
                foreach ($all_sessions_data as $session_number => $session_data) {
                    // Create virtual session ID
                    $session_id = $course_id * 100 + $session_number;
                    ...
                    if (isset($session_data['attendance'])) {
                        foreach ($session_data['attendance'] as $student_id => $status) {
                            $student_id = intval($student_id);
                            $status = sanitize($status);
                    ...
                    if (isset($session_data['participation'])) {
                        foreach ($session_data['participation'] as $student_id => $participations) {
                            $student_id = intval($student_id);
                ...
                jsonResponse(true, [
                    ...
                    'message' => 'All sessions data saved successfully'
                ], 'Data saved successfully');
                
            } catch (Exception $e) {
                $pdo->rollback();
                error_log("Save all sessions error: " . $e->getMessage());
                jsonResponse(false, [], 'Error saving data: ' . $e->getMessage());
            }
            break;
            
        case 'get_all_sessions_attendance':
            $course_id = intval($_GET['course_id'] ?? 0);
            
            error_log("Get all sessions attendance called for course_id: $course_id\n", 3, __DIR__ . "/../logs/api_errors.log");
